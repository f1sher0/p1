...
class MoleculeEncoder(nn.Module):
    def __init__(self, node_features, hidden_dim=256, output_dim=256, device='cpu'):
        super(MoleculeEncoder, self).__init__()
        self.conv1 = GCNConv(node_features, hidden_dim).to(device)
...
class myModel(nn.Module):
    def __init__(self, diag_voc_size, pro_voc_size, med_voc_size, med_voc, graph_data_dict, node_features=5, hidden_dim1=256, hidden_dim2=256,
    ...
    def _build_atc4_embedding(self, graph_data_dict, output_dim):
        ...
        """
        atc4_emb = {}
        for atc4, graph_data_list in graph_data_dict.items():
            smile_embeddings = []
            for graph_data in graph_data_list:
                if graph_data.edge_index is not None and graph_data.edge_index.numel() > 0:
                    with torch.no_grad():
                        emb = self.molecule_encoder(graph_data)  # 调用 MoleculeEncoder
                        smile_embeddings.append(emb)
            if smile_embeddings:
                atc4_emb[atc4] = torch.stack(smile_embeddings).sum(dim=0)  # 加和作为该 ATC4 的表示
            else:
                atc4_emb[atc4] = torch.zeros(output_dim)  # 如果没有图数据，初始化为零向量
        ...
        return nn.Parameter(atc4_matrix, requires_grad=False).to(self.device)
    ...
    def forward(self, input):
        ...
        # 匹配分数计算
        predictions = self.pred_layernorm(torch.sigmoid(torch.mm(patient_emb_proj, self.atc4_emb_matrix.t())))
        return predictions
